...
def run_prediction(
    ...
    When no market lines are available (lines_a=None, lines_b=None):
      - Falls back to a form + position + xG model with NO market anchor
      - Uses significantly higher weights on position/form to avoid flat equal priors
      - Applies conditional home advantage: reduced when away team is much stronger

    When market lines are available:
      - Market implied probability is the primary signal (55%)
      - Form, H2H, position are secondary corrections
    """
    if weights is None:
        weights = load_weights()

    has_lines = lines_a is not None or lines_b is not None

    # ── Market signal ─────────────────────────────────────────────────────
    if not has_lines:
        # No market lines: equal prior, then strongly overridden by
        # position/form/xG signals below
    ...
    # ── Position signal — MUCH stronger scaling ───────────────────────────
    # position 1 = top of table; lower number = stronger team
    # pos_diff > 0 means away team is LOWER (weaker); < 0 means away is HIGHER (stronger)
    pos_known = home_position is not None and away_position is not None
    if pos_known:
        # Normalise over 10 instead of 20 — twice as sensitive
        raw_pd = (away_position - home_position) / 10.0      # -1 to +1
        # Scale shift: 0.25 instead of 0.08 — 3x stronger effect
        pos_shift = raw_pd * 0.25
        ph_pos = min(max(0.33 + pos_shift, 0.05), 0.85)
        pp_pos = max(0.34 - abs(pos_shift) * 0.15, 0.08)   # draw less likely in lopsided matchups
        pa_pos = min(max(0.33 - pos_shift, 0.05), 0.85)
        tot_pos = ph_pos + pp_pos + pa_pos
        p_pos = (ph_pos / tot_pos, pp_pos / tot_pos, pa_pos / tot_pos)
    else:
        p_pos = p_mkt

    # ── Home advantage — conditional on relative strength ─────────────────
    # If away team is much higher in standings, reduce home advantage
    base_hfa = weights.get("w_home_field", 0.08)
    if pos_known:
        # pos_diff negative = away team is higher (stronger)
        strength_adj = max(0.0, min(1.0, 1.0 + (home_position - away_position) / 10.0))
        hfa = base_hfa * strength_adj          # shrinks toward 0 when away team is much stronger
    else:
        hfa = base_hfa
    ...
    diff    = hf_norm - af_norm                # -1 to +1 (positive = home better form)
    # Use conditional hfa here too
    fh = min(max(0.33 + diff * 0.25 + hfa, 0.05), 0.85)
    fx = min(max(0.34 - abs(diff) * 0.12,  0.05), 0.50)
    fa = min(max(0.33 - diff * 0.25 - hfa, 0.05), 0.85)
    tot = fh + fx + fa
    p_form = (fh / tot, fx / tot, fa / tot)

    # ── xG proxy — uses real scoring data when available ──────────────────
    home_xg = (home_scored + away_conceded) / 2
    away_xg = (away_scored + home_conceded) / 2
    xg_diff = (home_xg - away_xg) / max(home_xg + away_xg, 0.1)
    xh = p_mkt[0] * (1 + xg_diff * 0.15)     # was 0.10 — stronger xG effect
    xd = p_mkt[1]
    xa = p_mkt[2] * (1 - xg_diff * 0.15)
    tot2 = xh + xd + xa
    ...
    if h2h and h2h.get("total", 0) > 0:
        ...
        p_h2h = p_pos if pos_known else p_mkt  # use position as H2H fallback

    # ── Weight selection ─────────────────────────────────────────────────
    if not has_lines:
        # No market lines — rely entirely on statistical signals
        # Position is the strongest prior without market data
        wm = 0.05   # tiny market weight (just equal prior)
        wf = 0.35   # form is highly informative
        wh = 0.15   # H2H (or position fallback)
        wp = 0.45   # position — dominant signal without lines
    else:
