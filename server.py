"""
server.py — Football Analytics MCP Server v2.1
═══════════════════════════════════════════════
Combines:
  • Web dashboard  → served at /  (HTML + REST API)
  • MCP server     → mounted at /mcp

Transport selection via MCP_TRANSPORT env var:
  stdio  — Claude Desktop (default)
  http   — Render / cloud hosting (serves both dashboard + MCP)
"""
...
from analytics_engine import (
    ...
    remove_margin, provider_margin,
    goal_market_probs, run_prediction, select_pick,
    confidence_tier, confidence_bar,
    recent_form_pts, scoring_averages, h2h_record, form_string,
)
from schedule_data import all_fixtures, fixture_by_id, FIXTURES, ScheduledMatch
...
async def _api(path: str, params: dict = None) -> dict:
    if not API_KEY:
        raise ValueError("FOOTBALL_DATA_API_KEY not configured. Register free at football-data.org/client/register")
    async with httpx.AsyncClient(timeout=15) as client:
        r = await client.get(f"{API_BASE}{path}", headers={"X-Auth-Token": API_KEY}, params=params or {})
        r.raise_for_status()
        return r.json()
...
class PredictFixtureInput(BaseModel):
    ...
    show_analytics: bool = Field(True)
...
class RecordOutcomeInput(BaseModel):
    model_config = ConfigDict(extra="forbid")
    fixture_id:    str = Field(..., description="Fixture ID used during prediction")
    actual_result: str = Field(..., description="H (home), D (draw), A (away)")
...
async def analytics_list_fixtures(country: Optional[str] = None,
    ...
    """List all fixtures in the schedule. Filter by country or league."""
    ...
    for f in sorted(fixtures, key=lambda x: x.kickoff):
        ...
        lines.append(f"{aid:>7}  {bid:>7}  {f.kickoff:>6}  {match:<44}  {f.league:<22}  {f.country}")
    return "\n".join(lines)
...
async def analytics_predict_fixture(params: PredictFixtureInput) -> str:
    """
    Generate a full 5-outcome prediction for a scheduled fixture.
    Outcomes: Home Win, Draw, Away Win, Over 2.5 Goals, Both Teams Score.
    """
    try:
        f = fixture_by_id(params.fixture_id)
        if not f:
            return f"❌ Fixture ID {params.fixture_id} not found. Run `analytics_list_fixtures` first."
...
Provider margin: **{r['margin_pct']}%**  ·  Consensus gap: **{r['consensus_gap_pct']}%**
...
async def analytics_bulk_predictions(params: BulkPredictInput) -> str:
    """Ranked 5-outcome predictions for all schedule fixtures."""
    try:
        fixtures = all_fixtures()
        ...
        lines.append("⚠️ Statistical analysis only. Use `analytics_record_outcome` to train the model.")
        return "\n".join(lines)
    except Exception as e:
        return _api_error(e)
...
async def analytics_live_prediction(params: LivePredictInput) -> str:
    """Deep 5-outcome prediction using live form and H2H data from football-data.org."""
    try:
...
| | {hname[:20]} | {aname[:20]} |
|--|--|--|
| Form (last 5) | {h_fe} | {a_fe} |
| Form Pts /15  | {h_form} | {a_form} |
| Avg Scored    | {h_sc} | {a_sc} |
| Avg Conceded  | {h_cn} | {a_cn} |
| xG            | {r['home_xg']} | {r['away_xg']} |
...
async def analytics_live_fixtures(params: LiveFixturesInput) -> str:
    """Fetch upcoming fixtures from football-data.org. Requires FOOTBALL_DATA_API_KEY."""
    try:
        ...
        if not matches:
            return f"No scheduled fixtures for {LEAGUE_NAMES.get(params.league.upper(), params.league)} in {params.days_ahead} days."
        lines = [f"## ⚽ {LEAGUE_NAMES.get(params.league.upper(), params.league)} — {len(matches)} fixtures\n",
                 f"{'ID':<10}  {'Date/Time (UTC)':<22}  {'Home':<26}  Away", "─" * 82]
        for m in matches:
...
async def analytics_model_report() -> str:
    """View accuracy statistics and model weight configuration."""
    stats = accuracy_report(); weights = load_weights()
    lines = [
...
# ─────────────────────────────────────────────────────────────────────────────
# Entry point — combined app (dashboard + MCP at /mcp)
# ─────────────────────────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    if transport == "http":
        ...
        # Combined Starlette app: dashboard at /, MCP at /mcp
