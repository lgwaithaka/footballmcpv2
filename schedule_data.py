...
def register_live_fixtures(matches: list[ScheduledMatch]) -> int:
    ...
    Skips any fixture with missing team names, duplicates, or matches
    already present in the static FIXTURES list.
    Returns the number of new fixtures added.
    """
    global _LIVE_FIXTURES, _LIVE_IDS
    added = 0

    # Build static key set once outside the loop (performance + safety)
    static_keys: set[str] = set()
    for f in FIXTURES:
        if f.home_team and f.away_team and f.date:
            static_keys.add(f"{f.home_team.lower()}|{f.away_team.lower()}|{f.date}")

    for m in matches:
        # Guard: skip any fixture where team names are missing, empty, or TBD
        if not m.home_team or not m.away_team or not m.date:
            continue
        if not isinstance(m.home_team, str) or not isinstance(m.away_team, str):
            continue
        if m.home_team.strip() in ("", "TBD") or m.away_team.strip() in ("", "TBD"):
            continue

        key = f"{m.home_team.lower()}|{m.away_team.lower()}|{m.date}"

        if key in _LIVE_IDS or key in static_keys:
            continue
