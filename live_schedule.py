...
# ─────────────────────────────────────────────────────────────────────────────
# Team standings cache — populated at startup, used by dashboard
# ─────────────────────────────────────────────────────────────────────────────

# Keyed by lowercase team name (or first word of name)
# Values: {position, points, played, scored, conceded, avg_scored, avg_conceded}
TEAM_STANDINGS: dict[str, dict] = {}
...
def _normalise_name(name: str) -> str:
    ...
    name = name.lower()
    for sfx in [" fc", " afc", " cf", " sc", " united", " city", " athletic",
                " athletico", " hotspur", " wanderers", " rovers", " albion"]:
        name = name.replace(sfx, "")
    return name.strip()


def fetch_team_standings() -> int:
    """
    Fetch current EPL + UCL standings and populate TEAM_STANDINGS.
    Called at startup before uvicorn starts. Returns number of teams loaded.
    UCL has group/knockout round tables; we take first available table.
    """
    if not API_KEY:
        return 0
    loaded = 0
    for league_id, league_name in STANDINGS_LEAGUES.items():
        data = _get_sync(f"/competitions/{league_id}/standings")
        if not data:
            continue
        standings_list = data.get("standings") or []
        if not standings_list:
            continue
        # Take the TOTAL standings table (type=TOTAL) first, else first entry
        table = None
        ...
        for row in table:
            ...
            key1 = _normalise_name(name)
            key2 = name.lower().split()[0]          # first word
            TEAM_STANDINGS[key1] = entry
            TEAM_STANDINGS[key2] = entry
            loaded += 1

    print(f"[live_schedule] Team standings loaded: {loaded} entries for {len(STANDINGS_LEAGUES)} leagues")
    return loaded


def get_team_standing(team_name: str) -> Optional[dict]:
    """Look up a team in the standings cache. Returns None if not found."""
    k1 = _normalise_name(team_name)
    k2 = team_name.lower().split()[0] if team_name else ""
    return TEAM_STANDINGS.get(k1) or TEAM_STANDINGS.get(k2)
