...
from live_schedule import fetch_live_scores, get_team_standing
...
def _predict_fixture(f: ScheduledMatch) -> dict | None:
    ...
    # Enrich with team standings when no market lines are available
    # This replaces the useless equal-priors fallback with real strength data
    home_pos = away_pos = None
    home_scored = home_conceded = away_scored = away_conceded = 1.2
    if la is None and lb is None:
        hs = get_team_standing(f.home_team)
        as_ = get_team_standing(f.away_team)
        if hs:
            home_pos      = hs["position"]
            home_scored   = hs["avg_scored"]
            home_conceded = hs["avg_conceded"]
        if as_:
            away_pos      = as_["position"]
            away_scored   = as_["avg_scored"]
            away_conceded = as_["avg_conceded"]
...
async def api_markets(request: Request) -> JSONResponse:
    """Fetch Polymarket crowd probabilities for all fixtures."""
    fixtures = []
    for f in all_fixtures():
        d = _predict_fixture(f)
        if d:
            fixtures.append(d)
    market_data = await fetch_market_data(fixtures)
...
.brier-cell{font-family:'JetBrains Mono',monospace;font-size:10px;color:var(--muted)}
/* Win-probability highlight on cards */
.win-pct-row{padding:0 17px 12px;display:flex;align-items:stretch;gap:8px}
.win-pct-block{flex:1;border-radius:8px;padding:10px 8px;text-align:center;position:relative;overflow:hidden}
.win-pct-block.home-blk{background:rgba(0,255,135,.08);border:1px solid rgba(0,255,135,.2)}
.win-pct-block.draw-blk{background:rgba(0,148,255,.07);border:1px solid rgba(0,148,255,.18)}
.win-pct-block.away-blk{background:rgba(255,82,82,.08);border:1px solid rgba(255,82,82,.2)}
.win-pct-num{font-family:'Orbitron',sans-serif;font-size:20px;font-weight:900;line-height:1}
.win-pct-num.home-num{color:var(--green)}
.win-pct-num.draw-num{color:var(--blue)}
.win-pct-num.away-num{color:var(--coral)}
.win-pct-label{font-family:'JetBrains Mono',monospace;font-size:8px;color:var(--muted);letter-spacing:.08em;text-transform:uppercase;margin-top:3px}
.win-pct-team{font-family:'DM Sans',sans-serif;font-size:10px;color:var(--text);margin-top:2px;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;padding:0 2px}
/* Polymarket panel */
.pm-panel{margin:0 17px 14px;background:rgba(0,0,0,.25);border:1px solid rgba(255,255,255,.07);border-radius:8px;overflow:hidden}
.pm-header{padding:7px 12px;background:rgba(255,255,255,.04);display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid rgba(255,255,255,.06)}
.pm-title{font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--muted);letter-spacing:.1em;text-transform:uppercase;display:flex;align-items:center;gap:6px}
.pm-dot{width:6px;height:6px;border-radius:50%;background:#8B5CF6;flex-shrink:0}
.pm-vol{font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--muted)}
.pm-row{padding:8px 12px;display:flex;gap:8px;align-items:center}
.pm-team-block{flex:1;display:flex;flex-direction:column;gap:2px}
.pm-away-block{align-items:flex-end;text-align:right}
.pm-team-name{font-family:'DM Sans',sans-serif;font-size:10px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.pm-pct{font-family:'Orbitron',sans-serif;font-size:15px;font-weight:700}
.pm-pct.pm-home{color:var(--green)}.pm-pct.pm-draw{color:var(--blue)}.pm-pct.pm-away{color:var(--coral)}
.pm-center{width:50px;flex-shrink:0;display:flex;flex-direction:column;align-items:center;gap:2px}
.pm-draw-pct{font-family:'Orbitron',sans-serif;font-size:12px;font-weight:700;color:var(--blue)}
.pm-draw-label{font-family:'JetBrains Mono',monospace;font-size:8px;color:var(--muted)}
.pm-diverg{padding:0 12px 8px;display:flex;align-items:center;gap:8px}
.pm-diverg-label{font-family:'JetBrains Mono',monospace;font-size:8px;color:var(--muted);letter-spacing:.06em;text-transform:uppercase}
.pm-diverg-val{font-family:'JetBrains Mono',monospace;font-size:10px;font-weight:600}
.diverg-low{color:var(--green)}.diverg-mid{color:var(--amber)}.diverg-high{color:var(--coral)}
.pm-link{margin-left:auto;font-family:'JetBrains Mono',monospace;font-size:9px;color:#8B5CF6;text-decoration:none;opacity:.7}
.pm-link:hover{opacity:1}
.pm-na{padding:10px 12px;font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--muted);text-align:center;letter-spacing:.08em}
/* Consensus bar */
.consensus-bar{height:4px;background:rgba(255,255,255,.05);border-radius:2px;margin:0 12px 8px;position:relative;overflow:visible}
.c-bar-model{height:100%;position:absolute;top:0;left:0;background:var(--green);border-radius:2px;opacity:.7}
.c-bar-pm{width:3px;height:10px;position:absolute;top:-3px;background:#8B5CF6;border-radius:1px}
...
let marketData={};  // Polymarket crowd data keyed by fixture id
...
async function loadData(){
  try{
    const [fr,sr,mr]=await Promise.all([
      fetch('/api/fixtures'),fetch('/api/stats'),fetch('/api/markets')
    ]);
    const fd=await fr.json(), sd=await sr.json();
    const md=await mr.json().catch(()=>({}));
    allFixtures=fd.fixtures||[];
    marketData=md.markets||{};
...
function teamColour(pickCode, side){
  ...
  if(code==='HD') return side==='home'?'tn-win':'tn-loss';
  if(code==='DA') return side==='away'?'tn-win':'tn-loss';
  if(code==='HA') return 'tn-win';
  return '';
}
...
function pmPanel(fid, home, away){
  const pm = marketData[fid];
  if(!pm) return '';  // market data not loaded yet

  const hasPm = pm.pm_home_pct !== null && pm.pm_home_pct !== undefined;
  if(!hasPm){
    return `<div class="pm-panel">
      <div class="pm-header">
        <div class="pm-title"><div class="pm-dot"></div>POLYMARKET CROWD</div>
      </div>
      <div class="pm-na">No active market found</div>
    </div>`;
  }

  const divVal  = pm.divergence || 0;
  const divCls  = divVal < 8 ? 'diverg-low' : divVal < 18 ? 'diverg-mid' : 'diverg-high';
  const divTip  = divVal < 8  ? 'Model agrees with crowd'
                : divVal < 18 ? 'Some divergence — notable difference'
                :               'High divergence — potential value spot';

  // Consensus bar: model home% on track, PM marker as overlay
  const modelPos = Math.min(pm.home_win_pct, 100);
  const pmPos    = Math.min(pm.pm_home_pct,  100);

  const volHtml = pm.pm_volume ? `<div class="pm-vol">Vol ${pm.pm_volume}</div>` : '';
  const linkHtml= pm.pm_url
    ? `<a class="pm-link" href="${pm.pm_url}" target="_blank" rel="noopener">↗ polymarket</a>`
    : '';
  ...
  return `<div class="pm-panel">
    ...
    <div class="pm-row">
      <div class="pm-team-block">
        <div class="pm-team-name">${home.split(' ')[0]}</div>
        <div class="pm-pct pm-home">${pm.pm_home_pct}%</div>
      </div>
      <div class="pm-center">
        <div class="pm-draw-pct">${pm.pm_draw_pct}%</div>
        <div class="pm-draw-label">DRAW</div>
      </div>
      <div class="pm-team-block pm-away-block">
        <div class="pm-team-name">${away.split(' ')[0]}</div>
        <div class="pm-pct pm-away">${pm.pm_away_pct}%</div>
      </div>
    </div>
    <div class="consensus-bar" title="Green=our model home win%, Purple marker=Polymarket home win%">
      <div class="c-bar-model" style="width:${modelPos}%"></div>
      <div class="c-bar-pm" style="left:calc(${pmPos}% - 1.5px)"></div>
    </div>
    <div class="pm-diverg">
      <div class="pm-diverg-label">DIVERGENCE</div>
      <div class="pm-diverg-val ${divCls}" title="${divTip}">${divVal.toFixed(1)}%</div>
      ${pm.consensus_home !== null ? `<div class="pm-diverg-label" style="margin-left:10px">CONSENSUS HOME</div><div class="pm-diverg-val" style="color:var(--text)">${pm.consensus_home}%</div>` : ''}
      ${linkHtml}
    </div>
  </div>`;
}
...
function cardHTML(f){
  ...
  return`<div class="card" data-fid="${f.id}">
    ...
    <!-- Win % blocks: big numbers for home/draw/away -->
    <div class="win-pct-row">
      <div class="win-pct-block home-blk">
        <div class="win-pct-num home-num">${f.home_prob.toFixed(0)}<small style="font-size:11px">%</small></div>
        <div class="win-pct-label">HOME WIN</div>
        <div class="win-pct-team">${f.home.split(' ')[0]}</div>
      </div>
      <div class="win-pct-block draw-blk">
        <div class="win-pct-num draw-num">${f.draw_prob.toFixed(0)}<small style="font-size:11px">%</small></div>
        <div class="win-pct-label">DRAW</div>
        <div class="win-pct-team">—</div>
      </div>
      <div class="win-pct-block away-blk">
        <div class="win-pct-num away-num">${f.away_prob.toFixed(0)}<small style="font-size:11px">%</small></div>
        <div class="win-pct-label">AWAY WIN</div>
        <div class="win-pct-team">${f.away.split(' ')[0]}</div>
      </div>
    </div>
    ...
    <!-- Polymarket crowd panel -->
    ${pmPanel(f.id, f.home, f.away)}
    <!-- Additional goal markets -->
    <div class="outcomes" style="padding-top:4px">
